...
# a=1.0, b=4 → T_c≈0.0943, T=0.06 → T/T_c≈0.64, moderate density ratio.
# With CS EOS, G is just a ψ scaling factor — thermodynamics depends on a,b,T.
TAU: float = 0.55
LAMBDA_TRT: float = 0.03
G_SC: float = -1.0                   # CS: G is a ψ scaling factor
PSI_TYPE: int = 2                    # PSI_CS — Carnahan-Starling
PSI_REF: float = 1.0                 # not used for PSI_CS
CS_A: float = 1.0                    # standard literature value
CS_B: float = 4.0                    # CS covolume (η = ρ)
...
# ---------------------------------------------------------------------------
# CS EOS coexistence (a=1.0, b=4, T=0.06): ρ_l≈0.35, ρ_g≈0.005
RHO_LIQUID: float = 0.35
RHO_GAS: float = 0.005
...
# Rendering
# ---------------------------------------------------------------------------
SSFR_THRESHOLD: float = 0.10       # CS liquid ≈ 0.35, gas ≈ 0.005
...
class TrtDropletCoalescence:
    ...
    def __init__(self, viewer: FluidViewerGL) -> None:
        ...
        print(f"TRT Droplet Coalescence (CS EOS): {n}³, tau={TAU}, G={G_SC}")
        ...
        print(f"  ρ_liquid={RHO_LIQUID}, ρ_gas={RHO_GAS}")
