...
"""TRT + Shan-Chen (CS EOS) droplet free-fall onto a deep liquid pool.
...
Physics: TRT (Two-Relaxation-Time) + Carnahan-Starling equation of state.
The CS EOS pseudopotential provides a more realistic thermodynamic foundation
than the standard exponential form, with tunable surface tension (via T) and
higher achievable density ratios (~100:1 vs ~18:1).
...
# ---------------------------------------------------------------------------
# Physics: TRT + Shan-Chen (Carnahan-Starling EOS)
# ---------------------------------------------------------------------------
# CS EOS: ψ = sqrt(2·(P_CS − ρ·c_s²) / (G·c_s²))
#   P_CS = ρ·T·(1+η+η²-η³)/(1-η)³ − a·ρ²  with η = b·ρ/4
#   T_c = 0.3773·a/b — MUST use T < T_c for phase separation.
#   a=1.0, b=4 → T_c≈0.0943, T=0.08 → T/T_c≈0.85, near-critical, stable.
#   Lower T gives larger density ratio but sharper (more unstable) interface.
#   With CS EOS, G cancels out of P_eff ≡ P_CS — thermodynamics purely from a,b,T.
TAU: float = 0.60                    # tested stable for CS droplet dynamics
LAMBDA_TRT: float = 0.03
G_SC: float = -1.0                   # G irrelevant for CS thermodynamics (verified)
PSI_TYPE: int = 2                    # PSI_CS — Carnahan-Starling
PSI_REF: float = 1.0                 # not used for PSI_CS
CS_A: float = 1.0                    # only a=1.0 works (a=0.5 T_c too low)
CS_B: float = 4.0
CS_T: float = 0.07                   # T/T_c=0.74, sweet spot from param sweep
SC_BOUNDARY_PSI: float = 0.005       # gas-like wall
GRAVITY: float = -0.001              # very gentle — CS is sensitive
OMEGA_REG: float = 0.5               # reg-TRT blend

# ---------------------------------------------------------------------------
# Droplet + pool geometry
# ---------------------------------------------------------------------------
# CS coexistence from param sweep (a=1.0, b=4, T=0.07): ρ_l≈0.34, ρ_g≈0.005
RHO_LIQUID: float = 0.34
RHO_GAS: float = 0.005
POOL_HEIGHT: int = 30
DROPLET_RADIUS: int = 30
DROPLET_CX_FRAC: float = 0.50
DROPLET_CY_FRAC: float = 0.50
DROPLET_CZ_FRAC: float = 0.8
DROPLET_VZ_INIT: float = -0.55         # gravity-driven (CS lower density needs gentle start)
TRANSITION_WIDTH: float = 2.5

# ---------------------------------------------------------------------------
# Rendering
# ---------------------------------------------------------------------------
SSFR_THRESHOLD: float = 0.10          # CS liquid ≈ 0.34, gas ≈ 0.005
...
class TrtDropletFall:
    """Droplet impact onto a liquid pool with TRT + CS EOS.

    A spherical droplet (r = 30 cells) is released from rest above a
    pre-equilibrated 30-cell-deep pool.  Gravity (gz = -0.005) accelerates
    it downward to impact, producing crown splash and crater formation.
    Uses the Carnahan-Starling equation of state for the pseudopotential.
    ...
    def __init__(self, viewer: FluidViewerGL) -> None:
        ...
        print(f"TRT Droplet Fall (CS EOS): {n}³, tau={TAU} (nu={nu:.4f}), lambda_trt={LAMBDA_TRT}, G={G_SC}")
        ...
        print(f"  CS EOS: a={CS_A}, b={CS_B}, T={CS_T} (T_c≈{0.3773*CS_A/CS_B:.4f})")
        print(f"  ρ_liquid={RHO_LIQUID}, ρ_gas={RHO_GAS}, sc_boundary_psi={SC_BOUNDARY_PSI}")
