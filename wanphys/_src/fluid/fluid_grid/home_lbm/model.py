...
"""Static configuration for the D3Q27 HOME-LBM solver."""
...
@dataclass
class HomeLbmModel(FluidGridModelBase):
    """Configuration with an explicit physical/lattice unit contract."""
    ...
    def __post_init__(self) -> None:
        super().__post_init__()
        if any(int(v) <= 0 for v in self.fluid_grid_res):
            raise ValueError(f"fluid_grid_res must contain positive integers, got {self.fluid_grid_res}")
        ...
        if len(self.body_acceleration) != 3 or not all(math.isfinite(v) for v in self.body_acceleration):
            raise ValueError("body_acceleration must contain three finite physical components")
    ...
    def shear_omega(self) -> float:
        return 1.0 / self.shear_relaxation_time
    ...
    def validate_step(self, dt: float) -> None:
        if not math.isclose(dt, self.time_step, rel_tol=1.0e-9, abs_tol=1.0e-12):
            raise ValueError(
                f"HOME-LBM step dt={dt} does not match configured physical time_step={self.time_step}; "
                "use explicit fluid substeps instead of silently changing lattice units"
            )
