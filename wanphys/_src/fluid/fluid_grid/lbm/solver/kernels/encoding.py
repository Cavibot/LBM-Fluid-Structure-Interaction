...
def enforce_population_admissibility_kernel(
    ...
    For an admissible equilibrium velocity, ``f <- feq + alpha*(f-feq)``
    preserves local density and momentum.  ``alpha`` is the largest common
    value in ``[0, 1]`` for which all populations reach ``population_floor``.
    A radial velocity limit is applied only outside the D3Q19 low-Mach domain;
    that emergency branch preserves density but deliberately damps momentum.
    """
