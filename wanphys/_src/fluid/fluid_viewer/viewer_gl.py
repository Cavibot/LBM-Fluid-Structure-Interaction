...
SSFR_DEPTH_BLUR_RADIUS = 10
SSFR_THICKNESS_BLUR_RADIUS = 12
SSFR_DEPTH_FALLOFF = 12.0
SSFR_THICKNESS_SCALE = 4.8
SSFR_REFRACTION = 0.022
SSFR_FRESNEL_POWER = 3.0
SSFR_SPECULAR_STRENGTH = 0.3
...
class ScreenSpaceFluidRenderer:
    ...
    void main()
    {
        ...
        float scene_depth = texture(scene_depth_tex, vUV).r;
        float scene_linear_depth = linearize_depth(scene_depth);
        float occlusion_bias = 1.0e-3;
        if (scene_depth < 1.0 && scene_linear_depth < fluid_depth - occlusion_bias)
        {
            FragColor = vec4(scene_color, 1.0);
            return;
        }
    ...
    # ------------------------------------------------------------------
    # Ray-march shaders for density-field rendering (LBM / grid fluids)
    # ------------------------------------------------------------------

    _ray_march_depth_fragment_shader = """
    #version 330 core
    in vec2 vUV;
    out float outDepth;
    ...
    void main()
    {
        // --- ray direction (same as Camera.get_world_ray) -------------------
        vec2 ndc = vUV * 2.0 - 1.0;
        vec3 world_dir = normalize(
            camera_front_world
            + camera_right_world * ndc.x * alpha * aspect
            + camera_up_world    * ndc.y * alpha
        );
        vec3 world_origin = camera_pos_world;

        // --- ray-AABB intersection (slab method) ----------------------------
        // Avoid 0/0 = NaN when a ray-direction component is zero AND the
        // origin lies exactly on the corresponding AABB face.  We compute
        // 1/dir safely and multiply instead of dividing.
        vec3 inv_dir = world_dir;
        inv_dir.x = (abs(inv_dir.x) > 1e-10) ? (1.0 / inv_dir.x) : 1e10;
        inv_dir.y = (abs(inv_dir.y) > 1e-10) ? (1.0 / inv_dir.y) : 1e10;
        inv_dir.z = (abs(inv_dir.z) > 1e-10) ? (1.0 / inv_dir.z) : 1e10;
        vec3 t0 = (volume_min_world - world_origin) * inv_dir;
        vec3 t1 = (volume_max_world - world_origin) * inv_dir;
        ...
        // --- ray-march to find first-hit surface ----------------------------
        ...
        for (int i = 0; i < 2000; ++i) {
            if (t > tmax || i >= max_steps)
                break;
            vec3 pos = world_origin + world_dir * t;
            vec3 tex_coord = (pos - volume_min_world) / volume_size;
            tex_coord = tex_coord.zyx;  // NumPy C-order: k varies fastest -> OpenGL width dim
            ...
            float d = texture(density_tex, tex_coord).r;
            // Sub-step refinement: linearly interpolate the crossing point.
            if (d > density_threshold) {
                float hit_t = t;
                if (i > 0 && prev_d < density_threshold) {
                    float frac = (density_threshold - prev_d) / max(d - prev_d, 1.0e-8);
                    hit_t = prev_t + (t - prev_t) * frac;
                }
                vec3 hit_pos = world_origin + world_dir * hit_t;
                vec4 view_pos = view * vec4(hit_pos, 1.0);
                outDepth = -view_pos.z;
                return;
            }
            prev_d = d;
    ...
    _ray_march_thickness_fragment_shader = """
    #version 330 core
    in vec2 vUV;
    ...
    void main()
    {
        ...
        vec3 inv_dir2 = world_dir;
        inv_dir2.x = (abs(inv_dir2.x) > 1e-10) ? (1.0 / inv_dir2.x) : 1e10;
        inv_dir2.y = (abs(inv_dir2.y) > 1e-10) ? (1.0 / inv_dir2.y) : 1e10;
        inv_dir2.z = (abs(inv_dir2.z) > 1e-10) ? (1.0 / inv_dir2.z) : 1e10;
        vec3 t0 = (volume_min_world - world_origin) * inv_dir2;
        vec3 t1 = (volume_max_world - world_origin) * inv_dir2;
        vec3 tmin_v = min(t0, t1);
        vec3 tmax_v = max(t0, t1);
        float tmin = max(max(tmin_v.x, tmin_v.y), tmin_v.z);
        float tmax = min(min(tmax_v.x, tmax_v.y), tmax_v.z);
        ...
        outThickness = thickness * thickness_scale;
    }
    """
    ...
    def __init__(self, viewer: ViewerGL, max_particles: int, particle_radius: float, device):
        ...
        # Density-field ray-march state
        self._density_field = None
        self._density_res = (0, 0, 0)
        self._density_origin = (0.0, 0.0, 0.0)
        ...
        self._ray_march_depth_program = None
        self._ray_march_thickness_program = None
    ...
    def set_density_field(
        ...
        When set (non-None), the renderer uses ray-marching instead of
        particle splatting.  The density field is a dense 3D array on the
        GPU (``wp.array3d(dtype=float)``).

        Parameters
        ----------
        density:
            ``(nx, ny, nz)`` density array on device, or ``None`` to
            clear and fall back to particle rendering.
        grid_origin:
            World-space coordinate of the grid's minimum corner (x_min,
            y_min, z_min).
        cell_size:
            Grid cell size *dh* (world units).
        threshold:
            Density value above which a cell is considered "fluid"
        ...
        if density is not None:
            self._density_res = density.shape
            self._density_origin = tuple(grid_origin)
    ...
    def render(self, viewer: ViewerGL):
        has_particles = (
            self._particle_positions is not None and self._particle_count > 0
        )
        has_density = self._density_field is not None

        if self._failed or (not has_particles and not has_density):
            return
        ...
        try:
            ...
            if has_density:
                self._upload_density_texture()
                self._render_ray_march_passes()
            else:
                self._upload_particles(self._particle_positions)
                self._render_particle_passes()
    ...
    def _ensure_initialized(self):
        ...
        # Ray-march programs (for density-field rendering)
        self._ray_march_depth_program = compile_program(
            self._fullscreen_vertex_shader,
            self._ray_march_depth_fragment_shader,
        )
        self._ray_march_thickness_program = compile_program(
            self._fullscreen_vertex_shader,
            self._ray_march_thickness_fragment_shader,
        )

        # 3D texture for density field (lazy-sized, recreated on first upload)
        self._density_tex_3d = gl.GLuint()
        gl.glGenTextures(1, self._density_tex_3d)
        ...
        self._uniforms = {
            ...
            "ray_march_depth": self._collect_uniforms(
                self._ray_march_depth_program,
                "density_tex",
                "view",
                "camera_pos_world",
                "camera_front_world",
    ...
    # ------------------------------------------------------------------
    # Density-field ray-march rendering path
    # ------------------------------------------------------------------
    ...
    def _render_ray_march_passes(self):
        """Render depth and thickness via ray-marching the 3D density texture."""
        gl = self._gl
        renderer = self.viewer.renderer
        camera = self.viewer.camera
        width, height = self._frame_size
        nx, ny, nz = self._density_res
        dh = self._density_cell_size
        ox, oy, oz = self._density_origin

        # Camera basis vectors (same method as Camera.get_world_ray)
        view = np.ascontiguousarray(
            np.asarray(camera.get_view_matrix(), dtype=np.float32).reshape(4, 4)
        )
        proj = np.ascontiguousarray(
            np.asarray(camera.get_projection_matrix(), dtype=np.float32).reshape(4, 4)
        )
        texel = np.array((1.0 / width, 1.0 / height), dtype=np.float32)

        fov_rad = float(np.radians(camera.fov))
        alpha = float(np.tan(fov_rad * 0.5))
        aspect = float(width) / float(height)

        front = np.asarray(tuple(camera.get_front()), dtype=np.float32)
        right = np.asarray(tuple(camera.get_right()), dtype=np.float32)
        up_v  = np.asarray(tuple(camera.get_up()), dtype=np.float32)
        cam_pos = np.asarray(tuple(camera.pos), dtype=np.float32)

        volume_max = np.array(
            [ox + nx * dh, oy + ny * dh, oz + nz * dh], dtype=np.float32
        )
        volume_min = np.array([ox, oy, oz], dtype=np.float32)

        # ---- Pass 1: ray-march depth ----------------------------------
        gl.glBindFramebuffer(gl.GL_FRAMEBUFFER, self._depth_fbo)
        ...
        gl.glUseProgram(self._ray_march_depth_program.id)

        u = self._uniforms["ray_march_depth"]
        gl.glActiveTexture(gl.GL_TEXTURE0)
        gl.glBindTexture(gl.GL_TEXTURE_3D, self._density_tex_3d)
        gl.glUniform1i(u["density_tex"], 0)
        gl.glUniformMatrix4fv(u["view"], 1, gl.GL_FALSE, _arr_pointer(view))
        gl.glUniform3f(u["camera_pos_world"], *cam_pos)
        gl.glUniform3f(u["camera_front_world"], *front)
        gl.glUniform3f(u["camera_right_world"], *right)
        gl.glUniform3f(u["camera_up_world"], *up_v)
        gl.glUniform1f(u["alpha"], alpha)
        gl.glUniform1f(u["aspect"], aspect)
        gl.glUniform3f(u["volume_min_world"], *volume_min)
        ...
        # ---- Pass 2: ray-march thickness ------------------------------
        gl.glBindFramebuffer(gl.GL_FRAMEBUFFER, self._thickness_fbo)
        gl.glClearColor(0.0, 0.0, 0.0, 0.0)
        gl.glClear(gl.GL_COLOR_BUFFER_BIT)
        gl.glDisable(gl.GL_DEPTH_TEST)
        gl.glDepthMask(False)
        gl.glUseProgram(self._ray_march_thickness_program.id)

        u = self._uniforms["ray_march_thickness"]
        gl.glActiveTexture(gl.GL_TEXTURE0)
        gl.glBindTexture(gl.GL_TEXTURE_3D, self._density_tex_3d)
        gl.glUniform1i(u["density_tex"], 0)
        gl.glUniform3f(u["camera_pos_world"], *cam_pos)
        ...
        # ---- Pass 3-4: blur + composite (reuse existing) --------------
    ...
    def _render_particle_passes(self):
        gl = self._gl
        renderer = self.viewer.renderer
        camera = self.viewer.camera
        width, height = self._frame_size
