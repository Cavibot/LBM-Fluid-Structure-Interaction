...
正式守恒账本是 `sum(mass)+sum(pending_excess)`；`phi` 是始终位于 `[0,1]` 的
几何/质量通量权重。P2 的 `mass_tmp/phi_tmp/mass_delta/received_excess` 由
solver-owned `VofMassTransport` 持有，不进入
...
该命令的 `auto` 策略为 `runtime-profile=device` 和
`steps-per-frame=4`。可显式覆盖：
...
null/headless 的 `auto` 策略为 `runtime-profile=strict`，保持逐步完整验收。
...
```text
strict   每步完整 host 验收；测试/冻结/headless acceptance
sampled  每 30-100 步完整 host 验收；默认 interval=60
device   设备 reduction；每步只回读 16 个 float64
off      仅 transaction 与 epoch 门禁
```
...
- [06-development-roadmap.md](06-development-roadmap.md)：P0-P8 与 FIX1 开发门禁；
