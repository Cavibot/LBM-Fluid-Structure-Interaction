...
_RIGID_MODEL_INTERNAL_BRIDGES: set[str] = {"as_newton_model", "from_newton"}
...
def _should_skip_member(
    ...
    obj_qualname: str = getattr(obj, "__qualname__", "")
    obj_func: Any = getattr(obj, "__func__", None)
    if obj_func is not None:
        obj_module = getattr(obj_func, "__module__", obj_module)
        obj_qualname = getattr(obj_func, "__qualname__", obj_qualname)

    if (
        name in _RIGID_MODEL_INTERNAL_BRIDGES
        and obj_module.endswith("rigid.model")
        and obj_qualname.startswith("RigidModel.")
    ):
        return True
